...
var sF$=(function(){
  ...
  FCLib$.onLoaded(function(){
    ...
    document.body.classList.add(FC$.isEH?"globalEH":"globalFC");
  });
  ...
  function fnShowProd(Price,OriginalPrice,Cod,iMaxParcels,ProductID,bMinPrice){
    ...
    if(typeof ParcelsInterests!="undefined"){
      ...
      if(ParcelsInterests[iMaxParcels-1]>0)sInterest=""; else sInterest=rk("payment-table-installments-month")+" <span class='home-price-breakline'></span>";
    }
    else{
      iMaxParcels=0;
    }
    ...
    if(Price!=OriginalPrice && !bMinPrice){
      ...
      if(iMaxParcels>1)sPrice+="  <div class=\"installments font-regular\"><strong>"+ iMaxParcels +"x</strong> "+ rk("payment-table-installments-of") +" <strong>"+ FCLib$.formatMoney(CalculateInstallment(Price,iMaxParcels),FC$.Currency) +"</strong>"+ sInterest +"</div>";
      sPrice+="</div>";
    }
    else{
      sPrice+="<div class=\"prices\">";
  ...
  function fnSlideshowStore(aBanners){
    ...
    for(i=0;i<numBannerHomeFC && i<aBanners.length;i++){
      oBanner=aBanners[i];
      oSlideshow.innerHTML+='<div class="carousel-banner"><a href="/banner-redir.ehc?idbanner='+ oBanner.bannerID +'&url='+ encodeURIComponent(oBanner.url) +'"><picture><source media="(max-width:1024px)" srcset="'+ oBanner.imageMob +'"><img alt="'+ oBanner.alt.replace(/"/g,'&quot;')+'" src="'+ oBanner.image+ '" width="1900" height="350"></picture><div class="container-banner-text"></div></a></div>';
    }
    FCLib$.execLazyLoad();
  }
  function fnSlideshowNew(aBanners){
    ...
    var bannerTextSlideshow =document.querySelectorAll(".container-banner-text");
    ...
    flkty.on( 'select', function( index ) {
      ...
      if(fnGetConfig("Show_Banner_Text") == true){
        for(i=0;i<bannerTextSlideshow.length;i++){
          if(index == 0)bannerTextSlideshow[0].innerHTML='<p class="first-banner-text">'+ rk("slideshow-banner1-text-first") +'</p><p class="second-banner-text">'+ rk("slideshow-banner1-text-second") +'</p><div class="button-banner-text">'+ rk("slideshow-banner1-text-button") +'</div>';
          else if(index == 1)bannerTextSlideshow[1].innerHTML='<p class="first-banner-text">'+ rk("slideshow-banner2-text-first") +'</p><p class="second-banner-text">'+ rk("slideshow-banner2-text-second") +'</p><div class="button-banner-text">'+ rk("slideshow-banner2-text-button") +'</div>';
        }
      }else{
        for(i=0;i<bannerTextSlideshow.length;i++){
          bannerTextSlideshow[i].style.display="none";
        }
      }
    });
  ...
  function fnChangeImages(sImagemProdPri,sImagemProdDet,sDescUrl,sProductId,sNomeProd){
    ...
    if(fnGetConfig("Change_Image_Hover_Home_ProductList")){   
      if (tagImgPri==""){
        document.getElementById(sIdCampo).innerHTML="<img height='300' src='/images/nd0.gif'>";
      }
      else {
        ...
        if(nLenghtImg<0){
          document.getElementById(sIdCampo).innerHTML="<a href="+ sDescUrl +"><img "+ sSrcLocal +"='"+ sImagemProdPri +"' src='" + sImagemProdPri + "' height='300' width='300' alt=\""+ countNameProduct +"\""+ sLazy +" class='flickity-lazyload'></a>";
        }
        else {
          var valImgDet=null;
          ...
          if(imgDet0==null){
            ...
            document.getElementById(sIdCampo).innerHTML="<a href=" + sDescUrl + "><img "+ sSrcLocal +"='" + imgDet0 + "'  src='" + imgDet0 + "' height='300' width='300' border=0 onmouseover=\"this.src='" + imgDet1 + "'\"  onfocus=\"this.src='" + imgDet1 + "'\"  onmouseout=\"this.src='" + imgDet0 + "'\"   onblur=\"this.src='" + imgDet0 + "'\"  alt=\""+ countNameProduct +"\""+ sLazy +"  tabIndex='0' class='flickity-lazyload'></a>";
          }
        }
      }
  ...
  function fnInsertVideoGridThumb(){
    if(FC$.Page=="Products"){
      ...
      function onOffVideoThumb(currentItem){
        ...
        var getDataSlide=document.querySelector('.zoom-gallery .zoom-gallery-slide[data-slide-id="'+ currentItem.getAttribute('data-slide-id') +'"]');
        if(getDataSlide)getDataSlide.classList.add('active');
        var getDataSlideID=currentItem.getAttribute('data-slide-id');
        ...
        if(getDataSlideID == "zoom"){
          if(getVideoDiv)getVideoDiv.style.display="none";
        }else{
          if(getVideoDiv)getVideoDiv.style.display="block";
          if(getGridVideoIframe)getGridVideoIframe.style.display="block";
        }
      }
  ...
  function fnInsertDynCarouselCrossSelling(){
    if(fnGetConfig("Carousel_Cross_Selling")){
      ...
      if(elementCrossSellingID){
        ...
        document.querySelectorAll("#idListProdCrossFC .EstFotoProdCross").forEach(oImg=>{var sImg=oImg.getAttribute("data-src");oImg.removeAttribute("data-src");oImg.setAttribute("data-flickity-lazyload",sImg);oImg.setAttribute("width","152");oImg.setAttribute("height","152");});
  ...
  /* Selected Language */
  FCLib$.onReady(function(){fnShowSelectedLang();});
  function fnShowSelectedLang(){
    "use strict";
    if(fnGetConfig("Show_Language_Select")){
      var oLanguage=document.getElementById("header-language");
      if(oLanguage){
        showThis(oLanguage);
        var oOption=document.querySelectorAll(".langOpt"),iOption=oOption.length,iLang=(FC$.CustomLanguage?FC$.CustomLanguage:FC$.Language),bFound=false;
        for(var i=0;i<iOption && !bFound;i++)if(oOption[i].value==iLang){oOption[i].selected=true;bFound=true;}
      }
    }
  }

  function fnSetLang(este){
    "use strict";
    var iLang=este[este.selectedIndex].value;
    FCLib$.SetCookie("customlang",iLang);
    window.location.href=window.location.href.replace(/customlang/gi,"cl").replace(/#/,"?na#");
  }

  function showThis(obj){if(obj)obj.classList.remove("hidethis");}
  function hideThis(obj){if(obj)obj.classList.add("hidethis");}
  
  /* Fastcommerce/ehcommerce Badge */
  function fnFastcommerceBadgeSize(){  
    var imageBadgeFastcommerce = document.querySelector("#idBadgeAllFC img");
    imageBadgeFastcommerce.setAttribute('width','80');
    imageBadgeFastcommerce.setAttribute('height','25');
  }
  
  /* Shows cards and installments */
  function showTablePaymentsDetails(){
    var oPaymentsDetails=document.getElementById("tpDisplayCard");if(oPaymentsDetails && sF$.fnGetConfig("Product_Details_Payments_Cards"))oPaymentsDetails.classList.remove("hidethis");
  }
  
  /* Shows WhatsApp Buy Button */ 
  function fnWhatsAppBuyButton(prodId){
    var showWhatsAppButton=document.querySelectorAll(".container-whatsapp"+prodId);
    if(fnGetConfig("Show_BuyWithWhatsApp")){
      document.body.classList.add("buyWithWhatsApp");
      for(var i=0;i<showWhatsAppButton.length;i++)showWhatsAppButton[i].innerHTML='<a href="https://'+ (window.matchMedia('(min-width: 1024px)').matches?'web':'api') +'.whatsapp.com/send?phone=+'+ sF$.fnGetConfig("Phone_WhatsApp") +'&text='+ rk("theme-5-text-buy-whatsapp") +' http://lvfc.co/l'+ FC$.IDLoja +'/p'+ prodId +' " target="_blank"><img src="'+FC$.PathImg +'ic_whatsapp.svg?cccfc=1" width=24 height=24 alt="WhatsApp Buy"'+ sLazy +'><div>'+ rk("theme-5-buy-whatsapp") +'</div></a>';
    }
  }
  
  /* Header dropdown menu */
  function fnHeaderDropdown(){
    var menuHorCont=document.getElementById('horizontalMenuCont');
    for(var i in FCLib$.oCatMenu.slice(0,8)){
      var textTOInsert="";
      textTOInsert+='<li class="navigation-item">';
      textTOInsert+=' <a href="'+FCLib$.oCatMenu[i].url+'" class="navigation-link">'+FCLib$.oCatMenu[i].name+'</a>';
        if(FCLib$.oCatMenu[i].subs){
          textTOInsert+='<ul class="navigation-dropdown navigation-submenu" style="right: auto;"><span class="dm-arrow"></span>';
          for(var ii in FCLib$.oCatMenu[i].subs)textTOInsert+='<li class="navigation-dropdown-item"><a href="'+FCLib$.oCatMenu[i].subs[ii].url+'" class="navigation-dropdown-link">'+FCLib$.oCatMenu[i].subs[ii].name+'</a></li>';
          textTOInsert+='</ul>';
        }
        textTOInsert+='</li>';
        menuHorCont.innerHTML+=textTOInsert;
     }
  }
  
  /* Try to use the logo from config.js if it exists. Else use sTag (onerror) */
  function fnSetConfigLogo(obj,sTag){obj.onerror=null;obj.src=(fnGetConfig("Logo_Filename")?FC$.PathImg + fnGetConfig("Logo_Filename"):sTag);}
  
  /* Footer Company Address */
  function fnShowCompanyInfo(){
    if(!fnGetConfig("Show_CompanyAddress"))document.body.classList.add("showCompanyAddress");
    if(!fnGetConfig("Show_CompanyPhone"))document.body.classList.add("showCompanyPhone");
    if(!fnGetConfig("Show_CompanyEmail"))document.body.classList.add("showCompanyEmail");
    document.querySelectorAll("var[key=footer-cnpj]").forEach(function(obj){if(obj.nextSibling && obj.nextSibling.textContent.indexOf("/")<0 && rk("footer-cnpj")=="CNPJ")obj.innerText="CPF";});
  }
  
  /* Carousel Dynamic Height */
  function fnCarouselBannerHeight(){
    var containerCarousel=document.querySelector(".carouselBanner");
    var imagesChildContainer=containerCarousel.querySelectorAll("img");
    var maxHeightContainer=0;
    imagesChildContainer.forEach(img=>{
      var heightContainer=img.clientHeight;
      if(heightContainer>maxHeightContainer)maxHeightContainer=heightContainer;
    });
    containerCarousel.style.height=maxHeightContainer+"px";
  }
  
  /* Product Box Float buy estiloproduto.htm */
  function fnToggleBoxFloat(){
    var BoxFloat=document.getElementById("BoxFloat");
    var myScrollFunc=function(){
      var y=window.scrollY;
      if(y>=700)BoxFloat.style.display="block";
      else BoxFloat.style.display="none";
    };
    window.addEventListener("scroll",myScrollFunc);
    function closeDiv(){
      BoxFloat.style.display="none";
      window.removeEventListener("scroll",myScrollFunc);
    }
    var closeButton=BoxFloat.querySelector(".buyBoxFloatClose img");
    closeButton.onclick=closeDiv;
  }
  
  /* Product Det Show Review */
  function fnShowReviewsDet(urlProductReview,prodId,prodAverageReviews,prodReviewsQty){
    ...
    for(i=0;i<showQtdReviewStars.length;i++){
      if(iQtdOp==0){showQtdReviewStars[i].innerHTML="<a href='"+ urlProductReview +""+ prodId +"' aria-label='review'><div class='ProductStyleReviewOffBoxFC'><div class='NotaOpiniaoVazio NotaOpiniaoVazio1'></div><div class='NotaOpiniaoVazio NotaOpiniaoVazio2'></div><div class='NotaOpiniaoVazio NotaOpiniaoVazio3'></div><div class='NotaOpiniaoVazio NotaOpiniaoVazio4'></div><div class='NotaOpiniaoVazio NotaOpiniaoVazio5'></div><div class='NotaOpiniaoVazio NotaOpiniaoVazio6'></div></div></a>";}
      else{showQtdReviewStars[i].innerHTML="<a href='"+ urlProductReview +""+ prodId +"' aria-label='review'><div class='ProductStyleReviewOnBoxFC'><div class='ProductStyleReviewStarsOnFC'>"+ prodAverageReviews +"</div></a><div class='ProductStyleReviewQtyOnFC'>(<a href='#OP' style='text-decoration:none;'>0"+ prodReviewsQty +"</a>)</div></div>";}
    }
  }
  
  /* Product Det Share Social Media box */
  function fnshareBoxOpenClose(){
  ...
  /* Payments Icons */
  function fnShowPaymentsIconsFooter(){
    if(fnGetConfig("Show_PaymentsIcons")){
      var icPayments=fnGetConfig("PaymentsIcons_Filenames");
      var ulStructureLI=document.querySelector(".FooterULPaymentFC");
      for(var i=0;i<icPayments.length;i++){
        var liStructure=document.createElement("li");
        var imgPayment=document.createElement("img");
        imgPayment.setAttribute("src",FC$.PathImg+icPayments[i]);
        imgPayment.setAttribute("alt",icPayments[i].split('.').slice(0,-1).join('.'));
        imgPayment.setAttribute("loading","lazy");
        imgPayment.classList.add("PaymentIcon");
        liStructure.appendChild(imgPayment);
        ulStructureLI.appendChild(liStructure);
      }
    }
  }
  
  /* Dynamic Hero Image Login */
  function fnNewLoginPage(){
    if(fnGetConfig("Show_HeroImageInLogin")){
      document.body.classList.add("register-login-hero-image");
      var newDynamicDiv=document.createElement("div");
      newDynamicDiv.style.backgroundImage="url('"+ FC$.PathImg + sF$.fnGetConfig("LoginHero_Filename") +"')";
      newDynamicDiv.setAttribute("class","fc-login-hero-image-background");
      var existingIDPage=document.getElementById("idFCContent");
      existingIDPage.parentNode.insertBefore(newDynamicDiv,existingIDPage.nextSibling);
    }
  }
  ...
  return{
    ...
    showThis:showThis,
    hideThis:hideThis,
    fnFastcommerceBadgeSize:fnFastcommerceBadgeSize,
    showTablePaymentsDetails:showTablePaymentsDetails,
    fnWhatsAppBuyButton:fnWhatsAppBuyButton,
    fnHeaderDropdown:fnHeaderDropdown,
    fnSetConfigLogo:fnSetConfigLogo,
    fnShowCompanyInfo:fnShowCompanyInfo,
    fnCarouselBannerHeight:fnCarouselBannerHeight,
    fnToggleBoxFloat:fnToggleBoxFloat,
    fnShowReviewsDet:fnShowReviewsDet,
    fnshareBoxOpenClose:fnshareBoxOpenClose,
    fnshareBoxOpenIcons:fnshareBoxOpenIcons,
    showPaymentsFooter:showPaymentsFooter,
    fnShowPaymentsIconsFooter:fnShowPaymentsIconsFooter,
    fnNewLoginPage:fnNewLoginPage
  }
...
function fnFooter(){
  ...
  FCLib$.onReady(function(){
    ...
    sF$.fnFastcommerceBadgeSize();
    sF$.fnShowCompanyInfo();
    sF$.showPaymentsFooter();
    sF$.fnShowPaymentsIconsFooter();
    ...
    if(FC$.Page=="Products"){
      ...
      window.addEventListener("load",sF$.fnCarouselBannerHeight);window.addEventListener("resize",sF$.fnCarouselBannerHeight);
    }
...
function fnFooterPed(){
  fnShowYear();
  if(FC$.Step=="Login")sF$.fnNewLoginPage();
  FCLib$.onReady(function(){
    FCLib$.useLangResource(oResources);
    sF$.execLangResource();
  });  
...
function fnMaxInstallmentsGrid(PrecoProd,MaxParcelas){
  ...
  if(typeof ParcelsInterests!="undefined"){
    ...
    return "<div class=EstParc><b>"+MaxParcelas+ "" + rk("payment-table-no-interest-x") +"</b> "+ComSem+" "+ rk("details-price-no-interest-in") +" <b>"+ FCLib$.formatMoney(CalculateInstallment(PrecoProd,MaxParcelas),FC$.Currency) + rk("payment-table-installments-month") +"</b></div>";
  }else{
    return "";
  }
}
...
var dynCart$=(function(){
  ...
  if(bUseDC)document.body.classList.add("useDynCart");
  ...
  if(bUseDC)initDC();
  else{showMoreDetailsButton();FCLib$.onReady(function(){sF$.hideThis(document.querySelector("#product-details-dyncart-container"));});}
  return{
